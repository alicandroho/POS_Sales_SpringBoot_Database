...
public class ProductController {
	...
	public ProductModel putProduct(@RequestParam int productid, @RequestBody ProductModel newProductDetails)
			throws Exception {
		return pserv.putProduct(productid, newProductDetails);
	}

	@PutMapping("/putQuantity")
	public ProductModel putQuantity(@RequestParam int productid, @RequestBody ProductModel newProductDetails)
			throws Exception {
	...
	public ResponseEntity<ProductModel> getMostPurchasedProduct(@RequestParam String business) {
		// Call the service to retrieve the most purchased product for the given
		// business
	...
	public ResponseEntity<String> incrementPurchaseCount(@PathVariable int productid,
			@RequestParam int quantityPurchased) {
		try {
